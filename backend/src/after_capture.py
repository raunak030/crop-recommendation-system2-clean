"""
After Capture — Smart Crop Engine v1.5 Final Agronomic Hardening

Captures full results from MODIFIED compute_top_crops() (blended ranking + probability floor)
...
def main():
    ...
    for scenario in SCENARIOS:
        ...
        for rank, c in enumerate(top_crops, 1):
            # Compute blended ranking score for display
            ranking_score = 0.5 * c["model_probability"] + 0.5 * (c["suitability_score"] / 100.0)
            detail = {
                "rank": rank,
                "crop": c["crop_name"],
        ...
        all_results[scenario["name"]] = {
            ...
            "mode": "AFTER (blended ranking + 1% probability floor)",
        ...
        for entry in detail_list:
            ...
            print(f"    #{entry['rank']}: {entry['crop']:15s} prob={prob_pct:>7s}  "
                  f"suit={entry['suitability_score']}/100  "
                  f"blend={entry['ranking_score']:.4f}  unc={entry['uncertainty_label']}")
            if entry['coffee_penalty_applied'] > 0:
                print(f"           coffee_penalty={entry['coffee_penalty_applied']}")

    # Save after JSON
    output_path = Path(__file__).parent.parent / "data" / "baseline_after.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\n✅ Baseline AFTER saved to {output_path}")

    # Flag analysis
    print("\n\n=== FLAG ANALYSIS (AFTER) ===")
    print(f"{'Scenario':35s} {'<1% prob crops':30s} {'Coffee in top-5':20s} {'Coffee #1?':15s} {'0% prob in top-5':15s}")
    print("-" * 115)
    for sc_name, sc_data in all_results.items():
        low_prob = [c["crop"] for c in sc_data["top_crops"] if c["model_probability"] < 0.01]
        zero_prob = [c["crop"] for c in sc_data["top_crops"] if c["model_probability"] == 0.0]
        ...
        zero_str = ", ".join(zero_prob)[:15] if zero_prob else "(none)"
        coffee_str = f"YES (rank #{coffee_rank})" if coffee_in else "no"
        coffee_first = "YES" if coffee_rank == 1 else "no"
        print(f"{sc_name[:35]:35s} {low_str:30s} {coffee_str:20s} {coffee_first:15s} {zero_str:15s}")
